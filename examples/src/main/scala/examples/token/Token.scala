...
final class Token[F[_]: ConcurrentEffect: Timer] private (private val impl: Contract[F]) { self =>
  ...
  def subscribeTransfer: F[SubscriptionResult[F, Event]] = {
    for (result <- impl.subscribeLogs(Bytes32.from("0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"))) yield SubscriptionResult[F, Event](result.id, result.stream.map(decodeTransfer))
  }
  ...
  def subscribeApproval: F[SubscriptionResult[F, Event]] = {
    for (result <- impl.subscribeLogs(Bytes32.from("0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925"))) yield SubscriptionResult[F, Event](result.id, result.stream.map(decodeApproval))
  }
}
object Token {
