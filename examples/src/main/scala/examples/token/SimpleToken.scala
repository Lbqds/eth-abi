...
class SimpleToken(endpoint: String) { self =>
  ...
  def name(sender: Address, opt: TransactionOpt): Future[StringType] = {
    val encoded = Hex.hex2Bytes("06fdde03")
    impl.call(encoded, sender, opt).map { bytes => 
      val result = implicitly[TypeInfo[StringType]].decode(bytes, 0)
      result._1
    }
  }
  ...
  def totalSupply(sender: Address, opt: TransactionOpt): Future[Uint256] = {
    val encoded = Hex.hex2Bytes("18160ddd")
    impl.call(encoded, sender, opt).map { bytes => 
      val result = implicitly[TypeInfo[Uint256]].decode(bytes, 0)
      result._1
    }
  }
  ...
  def DECIMALS(sender: Address, opt: TransactionOpt): Future[Uint8] = {
    val encoded = Hex.hex2Bytes("2e0f2625")
    impl.call(encoded, sender, opt).map { bytes => 
      val result = implicitly[TypeInfo[Uint8]].decode(bytes, 0)
      result._1
    }
  }
  def INITIAL_SUPPLY(sender: Address, opt: TransactionOpt): Future[Uint256] = {
  ...
  def balanceOf(owner: Address, sender: Address, opt: TransactionOpt): Future[Uint256] = {
  ...
  def symbol(sender: Address, opt: TransactionOpt): Future[StringType] = {
  ...
  def allowance(owner: Address, spender: Address, sender: Address, opt: TransactionOpt): Future[Uint256] = {
  ...
  def subscribeTransfer: Source[EventValue, NotUsed] = {
    val query = LogQuery.from(contractAddress, Hash("0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"))
    impl.subscribeLogs(query).map(decodeTransfer)
  }
  ...
  def subscribeApproval: Source[EventValue, NotUsed] = {
    val query = LogQuery.from(contractAddress, Hash("0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925"))
    impl.subscribeLogs(query).map(decodeApproval)
  }
